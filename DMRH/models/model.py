...
class Model(nn.Module):
    def __init__(self, out_features, pretrain_model=None):
        ...
        self.classifier = nn.Sequential(
            nn.Conv2d(in_channels=4096, out_channels=out_features, kernel_size=1),
        )
        self.classifier.children().__next__().weight.data = torch.randn(out_features, 4096, 1, 1) * 0.01
        self.classifier.children().__next__().bias.data = torch.randn(out_features) * 0.01
        if pretrain_model:
